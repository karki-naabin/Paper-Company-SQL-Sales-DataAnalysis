/*Use the code given below to populate the database from the given parch-and-posey SQL file*/

psql -U postgres -d parch_and_posey -f C:\Users\Lenovo\Desktop\sqldatasets\parch-and-posey.sql -h localhost 


/* SQL statement to get data from column and limiting results to 10 rows can be extended to much more */

SELECT id, account_id, occurred_at -- * can be used to retrieve data from all columns
FROM orders
LIMIT 10;
...
FROM orders
ORDER BY total_amt_usd DESC -- in descending order otherwise result is in ascending order
LIMIT 5;
...
/*Write a query that displays the order ID, account ID, and 
total dollar amount for all the orders, sorted first by the account
...
/*Create a column that divides the standard_amt_usd by the standard_qty to find
 the unit price for standard paper for each order. Limit the results to the first 10 orders, and include the id and account_id fields.*/
...
/*Find all the company names that start with a
 'C' or 'W', and the primary contact contains 'ana' or 'Ana', but it doesn't contain 'eana'.*/
